...
{
  struct nvram_set_closure *c = closure;
  ...
  efivar = (struct efi_variable *) c->nvramptr;
  if (c->nvramptr - c->nvram + sizeof (struct efi_variable) > nvramsize)
    {
  ...
  c->nvramptr += sizeof (struct efi_variable);
  ...
  len = grub_utf8_to_utf16 ((grub_uint16_t *) c->nvramptr,
			    (nvramsize - (c->nvramptr - c->nvram)) / 2,
  ...
  c->nvramptr += 2 * len;
  *((grub_uint16_t *) c->nvramptr) = 0;
  c->nvramptr += 2;
  efivar->namelen = 2 * len + 2;

  if (unescape (var->value, c->nvramptr, c->nvram + nvramsize, &len))
    {
      efivar->namelen = 0;
      return 1;
    }
