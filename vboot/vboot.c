...
static grub_err_t
grub_cmd_vboot16 (struct grub_extcmd *cmd __attribute__ ((unused)),
		      int argc, char *argv[])
{	
	char *vhd_filename = 0;
...
/* insmod MODULE */
static grub_err_t
vboot_cmd_insmod (struct grub_extcmd *cmd __attribute__ ((unused)),
		      int argc, char *argv[])
{
  char *p;
  grub_dl_t mod;
...
/* insmod MODULE */
static grub_err_t
grub_cmd_vboot (struct grub_extcmd *cmd __attribute__ ((unused)),
		      int argc __attribute__ ((unused)), char *argv[] __attribute__ ((unused)))
{
  ...
  grub_file_t file = grub_file_open(vbootloader);
  if (file == 0)
  {
  ...
  if (mod)
  {
	...
	/* this might be already done inside menu.c */
	for (i=0; i<argc; i++)
	{
		//grub_printf("argv[%d]=%s\n", i, argv[i]);

		if (grub_strstr(argv[i], "config_file="))
		{
			done = 1;
			break;
		}
	...
	*/
	...
	if (config_file)
	{
		linux16_args[num_args] = config_file;
		num_args ++;
	}

	for (i=0; i<argc; i++)
	{
		linux16_args[num_args+i] = quote_string(argv[i]);
	}
...
int test_vhd_file(const char *pszFilename)
{
    ...
    grub_file_t file = grub_file_open(pszFilename);
	if (file == 0)
		rc = grub_errno;
    ...
    bytesRead = grub_file_read(file, buf, 512);
...
GRUB_MOD_INIT(vboot)
{
	cmd = grub_register_extcmd ("vboot16", grub_cmd_vboot16, GRUB_COMMAND_FLAG_BOTH, "vboot [OPTIONS...]", "vhd native boot", 0);
	cmd = grub_register_extcmd ("vbootinsmod", vboot_cmd_insmod, GRUB_COMMAND_FLAG_BOTH, "vboot [OPTIONS...]", "vhd native boot", 0);
	cmd = grub_register_extcmd ("vboot", grub_cmd_vboot, GRUB_COMMAND_FLAG_BOTH, "vboot [OPTIONS...]", "vhd native boot", 0);
	my_mod = mod;
}
