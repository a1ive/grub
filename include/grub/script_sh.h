...
struct grub_script_cmd_menuentry
{
  ...
  /* Options. 0 indicates menuentry, 1 is snapshotentry. */
  int options;

  /* child snapshots, nested, a child snapshot may contains its own snapshots */
  struct grub_script_cmd_menuentry *snapshots;

  void *private_data;

  /* a field to link a list of snapshots together, used for the above snapshots field
	 the next field should be NULL for menuentry 	
   */
  struct grub_script_cmd_menuentry *next;
};
...
{
  /* index to start snapshot.  */
  int startshot_pos;

  /* a list of snapshots belong to current menuentry or snapshotentry */
  struct grub_script_cmd_menuentry *snapshots;	
} scope_t;
...
scope_t* 
grub_enter_scope(struct grub_parser_param *state);
