...
grub_script_execute_menuentry (struct grub_script_cmd *cmd)
{
  ...
  grub_normal_add_menu_entry (argcount, (const char **) args,
			      cmd_menuentry->sourcecode, cmd_menuentry->options == 1, &pMenuEntry);  

  /* save the newly created menu entry, might be a snasphot */
  cmd_menuentry->private_data = pMenuEntry;
  ...
  snapshots = cmd_menuentry->snapshots;
  while (snapshots)
  {
	  grub_menu_entry_t snapshotEntry;
	  
	  grub_script_execute_menuentry((struct grub_script_cmd *)snapshots);
	  snapshotEntry = (grub_menu_entry_t)snapshots->private_data;

	  if (pMenuEntry->snapshots)
	  {
		// we need to append to the last
		struct grub_menu_entry *last_snapshot = pMenuEntry->snapshots;
		while (last_snapshot->next)
		{
			last_snapshot = last_snapshot->next;
		}
		last_snapshot->next = snapshotEntry;
	  }
	  else
	  {
		pMenuEntry->snapshots = snapshotEntry;
	  }
    	  
	  snapshotEntry->parent = pMenuEntry;
