			    ...
			    int options)
{
  ...
  scope = grub_current_scope(state);
  ...
  cmd->arglist = arglist;
  cmd->options = options;
  cmd->snapshots = scope ? scope->snapshots : NULL;
  cmd->next = NULL;
...
void push(stack_t **head, void *value)
{
    stack_t *node = grub_zalloc(sizeof(stack_t));  /* create a new node */

    if (node == NULL){
        grub_printf("Error: no space available for node\n");        
    } else {                                      /* initialize node */
        node->data = value;
        node->next = (*head == NULL) ? NULL : *head; /* insert new head if any */
        *head = node;
    }
}
...
void *pop(stack_t **head)
{
    if (*head == NULL) {                          /* stack is empty */
       grub_printf("Error: stack underflow\n");
	   return NULL;
    } else {                                     /* pop a node */
        stack_t *top = *head;
        void *value = top->data;
...
/* enter a new scope */
scope_t* grub_enter_scope(struct grub_parser_param *state)
{
	scope_t *s = (scope_t *)grub_zalloc(sizeof(scope_t));
	push(&state->scope_stack, s);

	return s;
}

void grub_leave_scope(struct grub_parser_param *state)
{
	scope_t *s = pop(&state->scope_stack);
	grub_free(s);
}

scope_t * 
grub_current_scope(struct grub_parser_param *state)
{
	if (state->scope_stack)
		return (scope_t *)state->scope_stack->data;
	else
		return NULL;
}

scope_t * 
grub_parent_scope(struct grub_parser_param *state)
{
	if (state->scope_stack)
	{
		stack_t *parent = state->scope_stack->next;
		if (parent)
			return (scope_t *)parent->data;
		else
			return NULL;
	}
	else
