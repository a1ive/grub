...
/* vboot real code is very small, so we can leave much more space for command line, 64K should be good enough */
#define GRUB_LINUX_CL_OFFSET_VBOOT	0x8000
		...
		int argc, char *argv[])
{
  ...
  slash = grub_strchr(argv[0], '/');
  is_vboot = slash && !grub_strcmp(slash, "/vboot/vboot");	
  ...
  if (lh.header == grub_cpu_to_le32 (GRUB_LINUX_MAGIC_SIGNATURE)
      ...
      if (grub_le_to_cpu16 (lh.version) >= 0x0201)
	{
	  ...
	  {
	    if (is_vboot)
           lh.cmd_line_ptr = grub_linux_real_addr + GRUB_LINUX_CL_OFFSET_VBOOT;
		else
      ...
      else
	{
	  ...
	  if (is_vboot)
        lh.cl_offset = grub_cpu_to_le16 (GRUB_LINUX_CL_OFFSET_VBOOT);
	  else
  ...
  if (is_vboot)
	dest = grub_stpcpy (grub_linux_tmp_addr + GRUB_LINUX_CL_OFFSET_VBOOT,
		      "BOOT_IMAGE=");
  else
