...
static int ventoy_update_before_chain(ventoy_os_param *param, char *isopath, char *script)
{
    grub_uint32_t jump_align = 0;
    ...
    if (g_wim_data.jump_bin_data)
    {
        ...
        ventoy_fill_windows_rtdata(g_wim_data.jump_bin_data + jump_align + sizeof(ventoy_os_param), isopath, script);
    }
...
grub_err_t ventoy_cmd_windows_chain_data(grub_extcmd_context_t ctxt, int argc, char **args)
{
    ...
    (void)ctxt;
    if (argc < 1)
      return grub_error (GRUB_ERR_BAD_ARGUMENT, "Usage: isopath [script]");
    ...
    {
        if (argc > 1)
            ventoy_update_before_chain(&(chain->os_param), args[0], args[1]);
        else
            ventoy_update_before_chain(&(chain->os_param), args[0], NULL);
    }
